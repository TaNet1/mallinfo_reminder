...
3. **邮件通知**：变动以美观的 HTML 邮件发送给配置的收件人；可选「无变动也发送」。
4. **可视化配置页**：
...
2. 填写 **③ 发件服务器 (SMTP)**：
   - 这是发邮件用的**发件邮箱账号**（必须由你提供）。
   - 例：QQ 邮箱 `smtp.qq.com` 端口 `465` 勾选 SSL；密码填邮箱的**授权码**（非登录密码）。
   - Gmail：`smtp.gmail.com` 端口 `465`，密码用应用专用密码。
3. 填写 **② 收件邮箱**（可多个）。
4. 点 **发送测试邮件** 验证 SMTP 配置正确。
5. 在 **① 发送周期** 选择间隔或每天定时，勾选 **开启定时任务**，点 **保存配置**。
